...
.Hardware Counters Environment Variables
[cols=",",options="header",]
|===
...
|TAU_OMPT_DISABLE_OFFLOAD|Set to disable OMPT callbacks for device-side offload events.
|TAU_CUPTI_PC_HWB|Set to change the size (in MB) of the buffer for CUPTI PC sampling.
|TAU_CUPTI_PC_PERIOD|Set to change the sampling perior(in cycles) for CUPTI PC sampling.
|TAU_USE_ROCSDK_SAMPLING_STOC|Force TAU to configure ROCm PC sampling in stochastic mode (enables stall events).
|TAU_USE_ROCSDK_SAMPLING_HOST|Force TAU to configure ROCm PC sampling in host-trap mode.
